...
class CSABlock(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        q = self.q_proj(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2) #  (B, N, L, C // N)
        k = self.k_proj(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2) #     
        v = self.v_proj(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2) # 

        # Causal self-attention
        # attention dropout
        ### YOUR CODE HERE ### 
        # N is the number of heads : self.n_head
        attn = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1))) # (B, N, L, L)
        attn = attn.masked_fill(self.mask[:, :, :L, :L] == 0, float('-inf')) # (B, N, L, L)
        ### YOUR CODE HERE ###
        attn = F.softmax(attn, dim=-1) # (B, N, L, L)
        attn = self.attn_drop(attn) # (B, N, L, L)
        ...
        return y, attn  # attn is returned for visualization purposes
